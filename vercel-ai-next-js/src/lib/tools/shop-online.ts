...
export const shopOnlineTool = withAsyncAuthorization(
  tool({
    description:
      "Tool to buy products online with human-in-the-loop authorization",
    parameters: z.object({
      product: z.string(),
      qty: z.number(),
      priceLimit: z.number().optional(),
    ...
    execute: async ({ product, qty, priceLimit }) => {
      ...
      if (accessToken) {
        headers["Authorization"] = "Bearer " + accessToken;
        console.log(
          "[shop-online] Access token found, adding Authorization header."
        );
      } else {
        console.log("[shop-online] No access token found.");
      }
      ...
      try {
        ...
        console.log(`[shop-online] Response status: ${response.status}`);
        console.log(
          `[shop-online] Response content-type: ${response.headers.get("content-type")}`
        );
        ...
        if (!contentType || !contentType.includes("application/json")) {
          ...
          console.log(
            `[shop-online] Non-JSON response: ${textResponse.substring(0, 200)}...`
          );
          throw new Error(
        ...
        return (
          ...
          `✅ Successfully ordered ${qty} ${product} for ${priceLimit ? `under $${priceLimit}` : "any price"}`
        );
      } catch (error) {
        console.error(
          "[shop-online] An error occurred during the API call:",
          error
        );
        throw error;
      }
